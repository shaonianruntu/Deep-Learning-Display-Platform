    ...
    return (
      <div className={style["SR"]}>
        <Row className={style["header"]}>
          <span className={style["title"]}>超分辨重建</span>
        </Row>
        <Row className={style["content"]}>
          ...
          {/* TODO: 当上传 input image 时，根据正则匹配其 ground truth，自动载入 */}
          <Card title="Ground Truth Image" style={{ marginLeft: "20px" }}>
            <div className={style["outputImage"]}>
              {/* {callback ? this.updateOutput() : ""} */}
            </div>
          </Card>
...
const mapState = state => ({
  imgUrl: state.getIn(["sr", "imgUrl"]),
  loading: state.getIn(["sr", "loading"]),
  callback: state.getIn(["sr", "callback"]),
  callBackInterval: state.getIn(["sr", "callBackInterval"])
});
